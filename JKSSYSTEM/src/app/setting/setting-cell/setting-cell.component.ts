...
@Component({
  selector: 'app-setting-cell',
  templateUrl: './setting-cell.component.html',
  styleUrls: ['../../table/table.component.css']
})
export class SettingCellComponent implements OnInit {

  displayedColumns: string[] = ['action', 'cell_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  openDialogDelete(data: any) {
    Swal.fire({
      ...
      if (result.value) {
        await this.deleteData(data.cell_code);
        this.getData = await getData();
        this.dataSource.data = await this.getData;
      }
    })
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadCell', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatCell', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteMaster/DeleteCellName/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/CellFormat', { headers: { Pragma: 'no-cache' } });
