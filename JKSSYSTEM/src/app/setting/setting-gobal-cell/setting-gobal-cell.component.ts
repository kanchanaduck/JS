...
@Component({
  selector: 'app-setting-gobal-cell',
  templateUrl: './setting-gobal-cell.component.html',
  styleUrls: ['../../table/table.component.css']
})
export class SettingGobalCellComponent implements OnInit {

  displayedColumns: string[] = ['action', 'gb_cell_code', 'gb_cell_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  openDialogDelete(data: any) {
    Swal.fire({
      ...
      if (result.value) {
        await this.deleteData(data.gb_cell_code);
        this.getData = await getData();
        this.dataSource.data = await this.getData;
      }
    })
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadGbCell', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatGobalCell', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteMaster/DeleteGbCellCode/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/GBCellFormat', { headers: { Pragma: 'no-cache' } });
