...
export class DetailModelComponent implements OnInit {
  displayedColumns: string[] = ['action', 'model_detail', 'process_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadModelProcess', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatDetail - Model', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteDetail/DeleteModelProcess/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getData() {
  try {
    const response = await axios.get(environment.apijks + '/Detail/ModelProcess', { headers: { Pragma: 'no-cache' } });
    return response.data;
  } catch (error) {
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/ModelProcess', { headers: { Pragma: 'no-cache' } });
