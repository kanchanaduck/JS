...
@Component({
  selector: 'app-setting-block-group',
  templateUrl: './setting-block-group.component.html',
  styleUrls: ['../../table/table.component.css']
})
export class SettingBlockGroupComponent implements OnInit {

  displayedColumns: string[] = ['action', 'block_group_code', 'block_group_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  openDialogDelete(data: any) {
    Swal.fire({
      ...
      text: 'ลบ : ' + data.block_group_detail,
      icon: 'warning',
      ...
      if (result.value) {
        await this.deleteData(data.block_group_code);
        this.getData = await getData();
        this.dataSource.data = await this.getData;
      }
    })
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadBlockGroup', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatBlockGroup', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteMaster/DeleteBlockGroup/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/BlockGroupFormat', { headers: { Pragma: 'no-cache' } });
