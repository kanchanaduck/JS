...
@Component({
  selector: 'app-setting-block-shift',
  templateUrl: './setting-block-shift.component.html',
  styleUrls: ['../../table/table.component.css']
})
export class SettingBlockShiftComponent implements OnInit {

  displayedColumns: string[] = ['action', 'shift_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  openDialogDelete(data: any) {
    Swal.fire({
      ...
      if (result.value) {
        await this.deleteData(data.shift_code);
        this.getData = await getData();
        this.dataSource.data = await this.getData;
      }
    })
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadBlockShift', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatBlockShift', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteMaster/DeleteBlockShift/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getData() {
  try {
    const response = await axios.get(environment.apijks + '/Master/BlockShift', { headers: { Pragma: 'no-cache' } });
    return response.data;
  } catch (error) {
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/BlockShiftFormat', { headers: { Pragma: 'no-cache' } });
