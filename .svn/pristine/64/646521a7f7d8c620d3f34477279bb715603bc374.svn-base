...
@Component({
  selector: 'app-detail-wc',
  templateUrl: './detail-wc.component.html',
  styleUrls: ['../../table/table.component.css']
})
export class DetailWcComponent implements OnInit {
  displayedColumns: string[] = ['action', 'wc_detail', 'model_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadWorkcenterModel', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatDetail - WorkCenter', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteDetail/DeleteWorkcenterModel/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getData() {
  try {
    const response = await axios.get(environment.apijks + '/Detail/WorkcenterModel', { headers: { Pragma: 'no-cache' } });
    return response.data;
  } catch (error) {
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/WorkcenterModel', { headers: { Pragma: 'no-cache' } });
