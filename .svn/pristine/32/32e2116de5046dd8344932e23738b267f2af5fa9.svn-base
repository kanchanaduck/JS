...
export class DetailCellComponent implements OnInit {
  displayedColumns: string[] = ['action', 'cell_detail', 'shift_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadCellShift', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatDetail - Cell', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteDetail/DeleteCellShift/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getData() {
  try {
    const response = await axios.get(environment.apijks + '/Detail/CellShift', { headers: { Pragma: 'no-cache' } });
    return response.data;
  } catch (error) {
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/CellShift', { headers: { Pragma: 'no-cache' } });
