...
@Component({
  selector: 'app-setting-cell-process',
  templateUrl: './setting-cell-process.component.html',
  styleUrls: ['../../table/table.component.css']
})
export class SettingCellProcessComponent implements OnInit {

  displayedColumns: string[] = ['action', 'process_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  openDialogDelete(data: any) {
    Swal.fire({
      ...
      if (result.value) {
        await this.deleteData(data.process_code);
        this.getData = await getData();
        this.dataSource.data = await this.getData;
      }
    })
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadProcess', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatProcess', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteMaster/DeleteProcessName/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getData() {
  try {
    const response = await axios.get(environment.apijks + '/Master/ProcessName', { headers: { Pragma: 'no-cache' } });
    return response.data;
  } catch (error) {
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/ProcessFormat', { headers: { Pragma: 'no-cache' } });
