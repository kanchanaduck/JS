...
@Component({
  selector: 'app-setting-wc',
  templateUrl: './setting-wc.component.html',
  styleUrls: ['../../table/table.component.css']
})
export class SettingWcComponent implements OnInit {

  displayedColumns: string[] = ['action', 'wc_detail'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  openDialogDelete(data: any) {
    Swal.fire({
      ...
      if (result.value) {
        await this.deleteData(data.wc_code);
        this.getData = await getData();
        this.dataSource.data = await this.getData;
      }
    })
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadWorkCenter', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatWorkCenter', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteMaster/DeleteWorkCenter/' + datas)
      this.getDataCSV = await getDataCSV();
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/WorkCenterFormat', { headers: { Pragma: 'no-cache' } });
