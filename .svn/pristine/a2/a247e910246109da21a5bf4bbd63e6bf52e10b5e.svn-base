...
@Component({
  selector: 'app-setting-model',
  templateUrl: './setting-model.component.html',
  styleUrls: ['../../table/table.component.css']
})
export class SettingModelComponent implements OnInit {

  displayedColumns: string[] = ['action', 'model_detail','model_child'];
  dataSource: any;
  getData: any = [];
  getDataCSV: any = [];
  ...
  openDialogDelete(data: any) {
    Swal.fire({
      ...
      if (result.value) {
        await this.deleteData(data.model_code);
        this.getData = await getData();
        this.dataSource.data = await this.getData;
      }
    })
  ...
  async Upload(formData: FormData) {
    try {
      ...
      this.loading = true;
      const response = await instance.post('/Upload/UploadModel', formData)
  ...
  exportFile(): void {
    ...
    this.columns = [];
    this.exportService.exportToCsv(element, 'FormatModel', this.columns);
  }
  // end upload data //
  ...
  async deleteData(datas: any) {
    try {
      ...
      const response = await instance.delete('/DeleteMaster/DeleteProductModel/' + datas)
      this.getDataCSV = await getDataCSV();
      Swal.fire({
        toast: true,
        position: 'top-end',
...
async function getData() {
  try {
    const response = await axios.get(environment.apijks + '/Master/ProductModel', { headers: { Pragma: 'no-cache' } });
...
async function getDataCSV() {
  try {
    const response = await axios.get(environment.apijks + '/Upload/ModelFormat', { headers: { Pragma: 'no-cache' } });
